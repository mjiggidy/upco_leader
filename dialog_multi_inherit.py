...
class Renderer(multiprocessing.Process):
	...
	def __init__(self, q, frame_black, frame_gray, frame_start, frame_count, width, height):
		super().__init__()
		print("Started")
		
		self.q = q
		
		self.frm_black = frame_black.copy()
		self.frm_gray  = frame_gray.copy()
		
		self.frm_start = frame_start
		self.frm_count = frame_count

		self.frm_width  = width
		self.frm_height = height
	...
	def run(self):
		...
		while not self.q.empty():
			print("Next!")
			x, frame_output = self.q.get()
			
			if x < ((self.frm_start + self.frm_count) - (2*24)):
				frame = self.frm_gray.copy()
			else:
				frame = self.frm_black.copy()

			# Draw countdown
			count = upco_leader.drawCountdown(radius=600,frame = x-self.frm_start)
			# Composite onto frame
			frame.alpha_composite(count, dest=(self.frm_width//2 - count.width//2, self.frm_height//2 - count.height//2))
			# Write to disk
			frame.save(str(frame_output))
			print(f"Done with {frame_output.name}")
...
class Settings(QtWidgets.QDialog):
	...
	def renderLeader(self):
		...
		btn_text = self.btn_ok.text()
		self.btn_ok.setText("Rendering...")
		self.update()
		...
		# Draw base frame
		frame_gray = Image.new("RGBA", (width, height), ImageColor.getrgb("rgba(128,128,128,255)"))
		frame_gray.alpha_composite(reticle)
		frame_gray.alpha_composite(star, dest=(400,200))
		frame_gray.alpha_composite(star, dest=(400, height-200-star.height))
		frame_gray.alpha_composite(star, dest=(width-400-star.width, 200))
		frame_gray.alpha_composite(star, dest=(width-400-star.width, height-200-star.height))

		# Draw black frame
		frame_black = Image.new("RGBA", (width, height), ImageColor.getrgb("rgba(0,0,0,255)"))

		# Prepare threaded rendering
		q = multiprocessing.Manager().Queue()
		for x in range(frame_start, frame_start + frame_count):
			q.put((x, path_output / f"{self.txt_title.text().strip()}_{width}x{height}.{str(x).zfill(6)}.tif"))

		self.prog_status.setRange(0, q.qsize())
		for _ in range(multiprocessing.cpu_count()):
			
			if not q.empty():
				print("Makin'")
				Renderer(q, frame_black=frame_black, frame_gray=frame_gray, frame_start=frame_start, frame_count=frame_count, width=width, height=height).start()
				print("Done makin'")



		while not q.empty():
			self.prog_status.setValue(q.qsize())

		self.btn_ok.setText(btn_text)
